              ...
              IF ( V_ESTOQUELOTE < :NEW.QTDNEG ) or (V_ESTOQUELOTE = 0) THEN
                 ...
                 -- INSERE NA TABELA DE LOG NOTA/PEDIDO SE TIVER DIVERGENCIA
                  IF INSERTING THEN
                       ...
                       V_MSG :=     'Não há estoque disponivel do produto :' || :NEW.CODPROD || ' , no local :' || :NEW.CODLOCALORIG ||' , empresa : ' || :NEW.CODEMP ||' e parceiro : '|| V_CODPARC  ||
                                    ' Estoque Atual: ' ||V_SALDO|| ' . E você está tentando movimentar  : '|| :NEW.QTDNEG ||'  ' ;
